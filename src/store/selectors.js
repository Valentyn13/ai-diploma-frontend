...
const mapMeditations = category =>
  category.meditations.map(({url, duration, _id, name, premium, createdAt, count, animation, thumbnail}) => ({
    url,
    duration,
    name,
    id: _id,
...
const findMeditation = (allMeditations, meditationId) => allMeditations.find(({id}) => id === meditationId);

export const allMeditations = createSelector([getCategories, getCourses], (categories, courses) => [
  ...categories.reduce((allMeds, category) => [...allMeds, ...mapMeditations(category)], []),
  ...courses.reduce((allMeds, course) => [...allMeds, ...mapMeditations(course)], []),
]);

export const allMeditationsButCourses = createSelector([getCategories, getCourses], (categories, courses) => [
  ...categories.reduce((allMeds, category) => [...allMeds, ...mapMeditations(category)], []),
]);
...
export const homeMeditationsSelector = createSelector(
  ...
  (categories, selectedCategories) => {
    ...
    let filteredCategories = categories.filter(({name}) => selectedCategories.includes(name.toLowerCase()));

    if (filteredCategories.length === 0) {
      filteredCategories = categories.length > 0 ? [categories[0]] : [];
    }

    return filteredCategories.reduce((meds, category) => [...meds, ...mapMeditations(category)], []);
  },
);
...
export const categoriesSelector = createSelector([getCategories], categories =>
  categories
    ...
    .map(category => {
      const {_id, name, title, info, order, isCategoryLocked, height} = category;
      return {
        id: _id,
        name,
        title,
...
export const homeCategoriesSelector = createSelector([getCategories], categories =>
  categories
...
export const practiceHistorySelector = createSelector(
  [allMeditations, state => state.userProgress.meditationsPracticed, state => state.userProgress.badgesAchieved],
...
export const latestMeditationSelector = createSelector([getCategories], categories => {
  const latest = categories.reduce((meds, category) => [...meds, ...mapMeditations(category)], []);
...
export const toptMeditationSelector = createSelector([getCategories], categories => {
  let topRated = categories.reduce((meds, category) => [...meds, ...mapMeditations(category)], []);
  // filter duplicates
  const names = topRated.map(item => item.name);
  topRated = topRated.filter((value, index) => names.indexOf(value.name) === index);
...
export const meditationInstructor = createSelector(
  ...
  (instructors, meditationId) => instructors?.find(instructor => instructor?.categories?.includes(meditationId)),
);
