...
כדי שאוכל לעזור בצורה הטובה ביותר, ספר/י לי בבקשה:
...
כדי שנתחיל את המסע מהמקום הנכון, שתף/י איתי בבקשה:
...
התפקיד שלי הוא לעזור לך לגלות את הכוחות הפנימיים שלך, לתת כלים ותובנות להתגבר על מכשולים, ולפתח תוכנית צמיחה הוליסטית שתקדם אותך צעד אחר צעד. אני מאמין בך ומחויב להיות השותף שלך במסע המרתק הזה של מימוש עצמי`;
  ...
  default: 'Content related to default category',
};
...
const getFirstMsg = (
  ...
  if (!category) {
    return getBasicMessage(name, gender);
  }
...
export const getFirstMsgs = (
  ...
  // @ts-ignore
  {
    ...
    user: SYSTEM_USER,
    createdAt: new Date(),
    text: getFirstMsg(name, gender, category),
  },
  {
    _id: 'intro_system_2',
    user: SYSTEM_USER,
